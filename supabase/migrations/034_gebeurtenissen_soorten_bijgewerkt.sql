-- 034 — De check op `werkbon_gebeurtenissen.soort` loopt weer gelijk
--       met wat de functies er daadwerkelijk in schrijven
--
-- Migratie 015 zette de gebeurtenissenlijst op met drie soorten:
-- 'stilgelegd', 'hervat' en 'opgeleverd'. Daarna zijn er vier
-- handelingen bijgekomen die óók een regel in dat logboek schrijven —
-- de ploeg wijzigen en de planning verzetten (030), een punt weghalen
-- (032), en een container of dixi stempelen (033) — maar de check is
-- nooit meegegroeid. Resultaat:
--
...
-- en het afvinken van een container. Vier features die sinds hun eigen
-- migratie geen enkele keer gewerkt kunnen hebben.
--
...
-- `useUitloop` filtert er hard op `soort = 'stilgelegd'` — een typfout
-- in een soort betekent daar stilzwijgend een gebeurtenis die niemand
-- meer terugvindt. Wat wél verandert is de volgorde van werken: wie
-- een nieuwe soort in een functie schrijft, zet hem hier in dezelfde
-- migratie erbij. De comment onderaan staat er om die afspraak in de
-- database zelf te laten staan, waar hij niet te missen is.
--
...
comment on constraint werkbon_gebeurtenissen_soort_check
  ...
  'Schrijf je een nieuwe soort in een functie? Zet hem hier in dezelfde migratie erbij. De insert is de laatste stap in die functies, dus een ontbrekende soort rolt de hele handeling terug in plaats van alleen de logregel.';
