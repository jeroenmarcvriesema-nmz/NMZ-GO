...
**Doel van dit document:** een volgende sessie moet dit project kunnen overnemen **zonder enig contextverlies**.

**Lees hoofdstuk 0a en 0 — dat is de actuele stand. Verder hoef je niet.** Wat daarna kwam (hoofdstuk 1 t/m 12) is op 14 augustus 2026 verwijderd omdat het een app beschreef die niet meer bestond; zie de notitie onderaan.

> **Twee dingen om te weten voor je hierop bouwt.**
>
> 1. **Cijfers verouderen sneller dan dit document wordt bijgewerkt.** Dat is hier al twee keer misgegaan. Zoek je een aantal — foto's, werkbonnen, opgeleverde klussen — vraag het dan op bij de database in plaats van het hier te lezen. Wat hier staat is gemeten op de datum die erbij staat, en niet later.
> 2. **Werk hoofdstuk 0 bij door het te hérschrijven, niet door er een hoofdstuk onder te hangen.** Dat laatste is precies hoe dit document 1053 regels lang werd waarin de bovenste en onderste laag elkaar tegenspraken.
...
**Tests.** Vitest, inmiddels 198 tests, `npm test`. Parser, planningsrekenwerk,
...
> **Bijgewerkt 14 augustus 2026.** De eerste twee punten hieronder waren
> achterhaald en zijn vervangen; ze stonden er nog toen de veldtest al
> liep, en een sessie heeft daar aantoonbaar verkeerde conclusies uit
> getrokken. De rest is opnieuw tegen de database gehouden.

- ~~**P0** Geen enkele foto ooit gemaakt.~~ **Achterhaald.** Er staan
  67 foto's, gemaakt door vier monteurs — Jeffrey (26), Danny (19),
  Justin (14) en Mario (4) — verdeeld over 4 werkbonnen. De fotoketen
  tot aan de opleverknop is daarmee in het veld bewezen.
- ~~**P0** Geen zwamsaneerder heeft ooit ingelogd.~~ **Achterhaald.**
  Er zijn 9 accounts: 1 eigenaar, 1 planner, 1 uitvoerder en 6
  medewerkers. Vier daarvan uploaden dagelijks.
- **P0** **Nog nooit een klus opgeleverd** — 0 van de 46. Dit is de
  nieuwe kern van het probleem: alles vóór de opleverknop werkt, alles
  eráchter is nog nooit één keer gelopen.
- **P0** PDF-generatie van het opleverrapport bestaat niet.
- **P1** ClickUp-terugkoppeling van de *oplevering* (status + opmerking)
  is nooit live uitgevoerd, alleen in droogloop. Ploeg en planning
  schrijven inmiddels wél echt terug (migratie 030).
- **P1** ClickUp attachment-upload bestaat niet. Drie foto's dragen een
  `clickup_geupload_op`-stempel, alle drie uit een handmatige test op
  11 augustus; de overige 64 zijn nooit naar ClickUp gegaan.
...
- **P1** `postcode`, `plaats` en `opdrachtnummer` zijn in de database
  overal leeg (0 van 46). Zoeken kijkt er sinds deze ronde wél in, maar
...
**Gemeten 14 augustus 2026** (de regel hieronder stond nog op de stand
van eind juli en klopte al een tijd niet meer):

46 werkbonnen · 634 afvinkpunten waarvan 41 afgevinkt · **67 foto's**
over 4 bonnen · 9 accounts (1 eigenaar, 1 planner, 1 uitvoerder, 6
medewerkers) · 0 opgeleverd · 0 opleverrapporten · 0 projecten ·
37 migratiebestanden in Git · 198 tests groen.

Twee dingen om uit deze cijfers te lezen:

1. **De veldtest loopt echt.** Vier monteurs uploaden dagelijks foto's
   op echte klussen. Alles tot aan de opleverknop is bewezen.
2. **41 van 634 punten afgevinkt, en nul opleveringen.** Het gebruik
   zit geconcentreerd op een handvol bonnen. De hele staart van de
   keten — attachment, statusterugkoppeling, opruimen, rapport — is
   nog volledig onbewezen.
...
# 1 t/m 12 — verwijderd, en waarom

Hier stonden twaalf hoofdstukken (bijna 300 regels) uit de periode
Sprint 2/3: projectoverzicht, technische architectuur, git-status,
de visie van de eigenaar, wat er misging in Sprint 3, de lessen
daaruit, en een aanbevolen volgende stap.

**Ze zijn verwijderd op 14 augustus 2026 omdat ze niet meer klopten en
actief schade deden.** Wat er stond, in de tegenwoordige tijd
geschreven alsof het de huidige stand was:

- "`001_initial.sql` — enige migratie tot nu toe" (het zijn er 37)
- "Sprint 2.1, enige commit in de git-historie"
- hooks die op mock data draaien (geen enkele meer)
- "de volledige premium UI-redesign is de belangrijkste openstaande
  prioriteit" (die is af)
- een `src/`-boom van vóór de helft van de huidige schermen

Dit is geen theoretisch risico. Een sessie die in augustus 2026 dit
document las, concludeerde dat er nog nooit een foto was gemaakt en
dat er nog nooit een monteur had ingelogd — terwijl er 67 foto's van
vier man in de database stonden. Dat kwam uit deze hoofdstukken plus
de toen nog niet bijgewerkte cijfers in hoofdstuk 0.

**De inhoud is niet weg**, hij staat in de git-historie:

```
git log --follow -p .ai/HANDOVER.md
```

**Waar de vervangende informatie staat:**

| Wat je zocht | Waar het nu staat |
|---|---|
| Actuele stand en cijfers | `PROJECT.md` → Huidige status, en hoofdstuk 0 hierboven |
| Architectuur, lagen, tabellen, sloten | `ARCHITECTURE.md` |
| Componenten | `COMPONENT_LIBRARY.md` |
| Merk- en designvisie van de eigenaar | `PRODUCT_VISION.md` |
| Wat er nog moet gebeuren | `FEATURE_BACKLOG.md` en `ROADMAP.md` |
| Functionele geschiedenis | `CHANGELOG.md` in de projectroot |
| Sprints en de fasen van Epic 4 | `SPRINTS.md` |

**Les voor wie dit document onderhoudt.** Een overdrachtsdocument dat
alleen maar aangroeit, wordt vanzelf een archeologische laagkoek waarin
de bovenste en de onderste laag elkaar tegenspreken — en een lezer kan
niet zien welke laag geldt. Schrijf hoofdstuk 0 opnieuw in plaats van er
een hoofdstuk onder te hangen, en gooi weg wat niet meer waar is. Git
onthoudt het toch.
