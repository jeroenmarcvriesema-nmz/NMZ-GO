...
- **Edge functions** (`supabase/functions/`) staan los van de Netlify-deploy en worden **apart** naar Supabase gezet. Een wijziging in `verwerker/`, `opdracht-lezen/` of `ploeg-bijwerken/` is dus pas live ná zo'n deploy, ook al staat de code op `main`.
- **Alle drie de functies lopen gelijk met deze repo** (`verwerker` v13, 12 augustus 2026). Bij een deploy van `verwerker` gaan álle bestanden mee: `index.ts`, `clickup.ts`, `werkopdracht.ts`, `ontleden.ts`, `register.ts`, `statusregels.ts`, `opruimen.ts`. Vergeet er één, dan start de functie niet meer.
- **Controleer een deploy altijd door de bestanden terug te halen** en te vergelijken met de repo. Dat is geen overdaad: bij de deploy van 12 augustus bleek zo dat zes bestanden byte-voor-byte klopten en er in het zevende één tekenklasse anders geschreven stond.
- **Bekend en onschuldig verschil:** in `clickup.ts` staat in de repo `/[̀-ͯ]/` (het wegstrepen van accenten in `bijlagenaam`), terwijl er in productie de letterlijke combineertekens staan. Dezelfde codepunten, dezelfde uitkomst — het komt doordat een deploy via de API door JSON gaat en die escape onderweg omzet. Een deploy via de Supabase CLI leest het bestand rechtstreeks en heeft dit niet. Laat je niet verrassen door een vergelijking die hierop "verschilt" meldt. Let op de padopbouw: `opdracht-lezen/index.ts` importeert `../verwerker/werkopdracht.ts`, dus de bestanden gaan mee met hun map ervoor (`opdracht-lezen/index.ts`, `verwerker/werkopdracht.ts`, `verwerker/ontleden.ts`) en het startpunt is `opdracht-lezen/index.ts`. Plat aanleveren breekt die import. De parser staat bewust op één plek: twee kopieën lopen uit de pas en dan geeft dezelfde opdracht twee verschillende takenlijsten.
