...
- **Let op: `verwerker` in productie loopt achter op deze repo.** Nog niet uitgerold, in volgorde van ouderdom: de handler `onderhoud.fotos_opruimen` (slaapt tot er zo'n taak in de wachtrij komt — dat gebeurt nu nergens), de stilleg-tekst naar ClickUp die niet meer een verschoven opleverdatum belooft (migratie 029 is wél al toegepast, dus productie meldt daar nu iets onjuists), en het bijwerken van het personenregister tijdens de ronde (`register.ts`). Tot die deploy komen nieuwe ClickUp-namen alleen binnen via de knop "Uit ClickUp ophalen". Bij die deploy gaan álle bestanden van de functie mee: `index.ts`, `clickup.ts`, `werkopdracht.ts`, `ontleden.ts`, `register.ts`, `statusregels.ts`, `opruimen.ts`. Let op de padopbouw: `opdracht-lezen/index.ts` importeert `../verwerker/werkopdracht.ts`, dus de bestanden gaan mee met hun map ervoor (`opdracht-lezen/index.ts`, `verwerker/werkopdracht.ts`, `verwerker/ontleden.ts`) en het startpunt is `opdracht-lezen/index.ts`. Plat aanleveren breekt die import. De parser staat bewust op één plek: twee kopieën lopen uit de pas en dan geeft dezelfde opdracht twee verschillende takenlijsten.
