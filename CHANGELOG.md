...
## De koppeling met ClickUp leest nu ook terug

- **[FEATURE]** Wat er in **ClickUp** met een klus gebeurt, komt nu terug in NMZ GO. Zet kantoor een taak op **opgeleverd**, **on hold**, **onhold door asbest**, **nog spuiten/isoleren** of **opnieuw inplannen/later**, dan volgt de werkbon. Tot nu toe was status eenrichtingsverkeer: NMZ GO schreef naar ClickUp en las er niets van terug. Dat werkt zolang iedereen in de app werkt, en dat doet nog niet iedereen — negen klussen stonden op "nog niet gestart" terwijl ze in ClickUp al gesloten waren, de oudste tien dagen. Het bord loog dus over het werk, en daar wordt op gestuurd.
- Er zaten **drie** oorzaken achter elkaar. De synchronisatie vraagt ClickUp alleen om taken op de triggerstatussen, dus een taak die op "opgeleverd" gaat staan valt uit beeld en wordt nooit meer bekeken. `haalTaken` weigerde bovendien gesloten taken sowieso — `include_closed=false` stond hardgecodeerd in de URL, waardoor "voeg opgeleverd toe aan de statuslijst" in zijn eentje niets zou hebben opgeleverd. En de status werd wel gelezen maar nergens toegepast.
- Daarom een **aparte ronde** (`clickup.standen_ophalen`) naast de bestaande, met één harde regel: **hij maakt nooit een werkbon aan.** Zou je "opgeleverd" aan de bestaande ronde toevoegen, dan komen er in één keer ruim tweehonderd historische taken binnen die stuk voor stuk een PDF-download en een nieuwe bon opleveren — van vierenzeventig bonnen naar driehonderd. Deze ronde kijkt alleen naar klussen die NMZ GO al kent, en is daarmee ook goedkoop: één lijstquery, en met `date_updated_gt` komt er meestal niets terug.
- **Opgeleverd is eenrichtingsverkeer.** Staat een klus in NMZ GO op opgeleverd, dan zet geen enkele ClickUp-status hem terug. Aan een oplevering hangt een opleverrapport en een fotoketen; die maak je niet ongedaan met een muisklik op een planbord.
- **Stilstand mag wél terug — maar alleen die van de koppeling zelf.** "On hold" is per definitie tijdelijk: kan de sync hem niet opheffen, dan staat elke klus die ooit even stillag voor altijd rood. Het onderscheid loopt via `stilgelegd_door`: leeg betekent dat de koppeling hem heeft gezet en hem dus ook weer mag weghalen, gevuld betekent dat een mens die afweging heeft gemaakt en dan blijft hij staan.
- **Spreken beide systemen elkaar tegen, dan wint niemand.** Ligt een klus in NMZ GO stil terwijl ClickUp "opgeleverd" zegt, dan wijzigt er niets en krijgt kantoor een melding in de klok — hooguit één keer per klus per etmaal, want de ronde draait elke vijf minuten.
- `wacht op foto's` wordt bewust **niet** teruggelezen. Die status zet NMZ GO zélf op het bord bij het opleveren als het fotobewijs nog niet bij ClickUp staat. Hem terugvertalen betekent je eigen echo inlezen, en dat is een lus die bij elke ronde een rondje verder draait. Er staat een test op.
- De vertaling van ClickUp-status naar stand staat als losse regel in `statusregels.ts`, naast `statusUitReden` die al de andere kant op deed, met **negen tests**. Ook op de statussen die géén betekenis krijgen: "toekomst", "niet af" en "update vereist" zijn planning van kantoor en zeggen niets over de uitvoering. Een gok is daar erger dan geen antwoord.
- Proefdraaien kan: een taak met `"droogloop": true` in de payload laat zien wát hij zou aanraken en schrijft niets. De hartslag zet die vlag nooit aan.
- Migratie 043. Geen nieuwe soort in `werkbon_gebeurtenissen` — dit gebruikt de vijf die er al in staan, dus de val uit migratie 039 speelt hier niet.
