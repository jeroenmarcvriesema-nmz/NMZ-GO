  ...
  return (
    <PageWrapper title="Werkbonnen" actions={
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Zoek op adres, project of medewerker…" value={zoek} onChange={(e) => setZoek(e.target.value)}
            className="w-full pl-9 pr-4 py-2.5 text-sm border border-gray-200 rounded-sm outline-none focus:border-brand-yellow" />
        </div>
        <div className="flex gap-1.5 bg-surface-2 p-1 rounded-sm">
          {statusFilters.map((f) => (
            <button key={f.value} onClick={() => setStatusFilter(f.value)}
              className={`px-3 py-1.5 rounded text-xs font-semibold transition-all ${statusFilter === f.value ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
              {f.label}
            </button>
          ))}
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-gray-400"><div className="text-4xl mb-3">📋</div><div className="font-medium">Geen werkbonnen gevonden</div></div>
      ) : (
